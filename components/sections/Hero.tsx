...
export default function Hero(): ReactElement {
  return (
    <AnimatedSection id="home" className="relative overflow-hidden bg-black" variants={container}>
      <div className="mx-auto max-w-[393px] sm:max-w-7xl px-4 sm:px-6 lg:px-12 pb-20 pt-28 md:pt-32">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:items-center">
          <motion.div variants={container} className="flex flex-col gap-6">
            ...
            <h1 className="display-heading text-white text-3xl sm:text-4xl md:text-5xl lg:text-6xl leading-tight">
            ...
            <motion.p variants={item} className="max-w-lg text-base md:text-lg text-zinc-300">
              Bridging creativity and functionality to deliver stunning, user-friendly web applications
            </motion.p>

            <motion.div variants={item} className="flex items-center gap-2">
              <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }} className="w-full sm:w-auto">
                <GradientButton className="w-full sm:w-[300px] h-[56px] rounded-full p-2 text-sm font-semibold justify-center gap-0 shadow-[0_8px_30px_rgba(16,185,129,0.18)] ring-4 ring-[#91FF02]/10">
                  HIRE ME
                </GradientButton>
              </motion.div>
            </motion.div>
          ...
          <motion.div variants={item} className="relative flex items-center justify-center md:justify-end">
            <motion.div className="relative w-full max-w-[361px] sm:max-w-[420px] md:max-w-[660px] h-auto overflow-hidden mx-auto md:ml-auto">
              <Image src="/profile-image.png" alt="Profile image" width={660} height={874} className="w-full h-auto object-cover rounded-none" priority />
